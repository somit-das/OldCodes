...
public class ViewStudent extends HttpServlet {
	protected void doGet(HttpServletRequest req, HttpServletResponse rs) throws ServletException, IOException {
		...
		pw.print("<br><h2 class='text-center' style='font-family: Times New Roman, Times, serif;font-weight:bold; text-shadow: 0 0 3px #FF0000, 0 0 5px #0000FF;'> All Student List <span class='bi bi-people-fill'></span></h2>");
		...
		pw.print("<thead class='bg-success text-white text-center rounded-2'><tr><th>Rollno</th><th>Name</th><th>Email</th><th>Sex</th><th>Course</th><th>Fee</th><th>paid</th><th>due</th><th>address</th><th>contact</th><th>Edit</th><th>Delete</th></thead>");
		for(StudentBean bean:list){
			pw.print("<tbody class='text-center'><tr><td>"+bean.getRollno()+"</td><td>"+bean.getName()+"</td><td>"+bean.getEmail()+"</td><td>"+bean.getSex()+"</td><td>"+bean.getCourse()+"</td><td>"+bean.getFee()+"</td><td>"+bean.getPaid()+"</td><td>"+bean.getDue()+"</td><td>"+bean.getAddress()+"</td><td>"+bean.getContact()+"</td><td><a style='background-color:blue;border:1px solid blue;padding:2px;border-radius:3px 5px;' href='EditStudentForm?rollno="+bean.getRollno()+"'>Edit</a></td><td><a style='background-color:red;border:1px solid red;padding:2px;border-radius:3px 5px;' href='DeleteStudent?rollno="+bean.getRollno()+"'>Delete</a></td></tr></tbody>");
		}
		pw.println("</table>");
		pw.println("</div><br>");
