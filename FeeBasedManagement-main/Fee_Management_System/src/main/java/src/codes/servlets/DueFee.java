...
public class DueFee extends HttpServlet {
	protected void doGet(HttpServletRequest req, HttpServletResponse rs) throws ServletException, IOException {
		...
		pw.println("<html>");
		pw.println("<head>");
		pw.println("<title>View Student</title>");
		...
		pw.print("<br><h2 class='text-center' style='font-family: Times New Roman, Times, serif;text-shadow: 0 0 3px #FF0000, 0 0 5px #0000FF; font-weight:bold;'>All Due Fee<span class='bi bi-currency-rupee'></span></h2>");
		...
		pw.print("<thead class='bg-primary text-white text-center rounded-2'><tr><th>Rollno</th><th>Name</th><th>Email</th><th>Sex</th><th>Course</th><th>Fee</th><th>Paid</th><th>Due</th><th>Address</th><th>Contact</th><th>Edit</th><th>Delete</th></thead>");
		for(StudentBean bean:list){
			pw.print("<tbody class='text-center'><tr><td>"+bean.getRollno()+"</td><td>"+bean.getName()+"</td><td>"+bean.getEmail()+"</td><td>"+bean.getSex()+"</td><td>"+bean.getCourse()+"</td><td>"+bean.getFee()+"</td><td>"+bean.getPaid()+"</td><td>"+bean.getDue()+"</td><td>"+bean.getAddress()+"</td><td>"+bean.getContact()+"</td><td><a <a style='background-color:green;border:1px solid green;padding:2px;border-radius:3px 5px;' href='EditStudentForm?rollno="+bean.getRollno()+"'>Edit</a></td><td><a <a style='background-color:red;border:1px solid red;padding:2px;border-radius:3px 5px;' href='DeleteStudent?rollno="+bean.getRollno()+"'>Delete</a></td></tr></tbody>");
		}
