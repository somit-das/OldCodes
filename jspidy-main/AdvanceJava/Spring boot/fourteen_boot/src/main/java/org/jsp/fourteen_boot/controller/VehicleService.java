...
public class VehicleService {
	...
	public ResponseStructure<String> deleteByIdController(@RequestParam int id){
		...
		if(op.isEmpty()) {
			...
			vdao.deleteByIdDAO(id);
			op=vdao.findSpecificVehicleDAO(id);
			if(op.isPresent()) {
				str.setStatusCode(500);
				str.setStatusMesg("Unable to Perform this Action ");
	...
	public ResponseStructure<String> deleteAllService() {
		List<Vehicle> vehiclelist =vdao.findAllVehiclesDAO();

		ResponseStructure<String> str = new ResponseStructure<>();
		if(vehiclelist.size()>0) {
			vdao.deleteALLDAO();
			vehiclelist = vdao.findAllVehiclesDAO();
			if(vehiclelist.size()==0) {
				str.setStatusCode(204);
				str.setStatusMesg("Successfull");
				str.setResbody("Able to Delete Each Vehicle Data...... ");
			}
			else {
				str.setStatusCode(500);
				str.setStatusMesg("Not Successfull");
				str.setResbody("Not Able to Delete The Vehicle List... ");
			}
		}else {
			str.setStatusCode(404);
			str.setStatusMesg("Not Successfull");
			str.setResbody("No Vehicle Found in List ");
		}
		return str;
	}
